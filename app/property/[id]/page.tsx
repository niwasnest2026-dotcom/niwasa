...
export default function PropertyDetails() {
  ...
  // Handle booking attempt - check authentication first
  const handleBookingAttempt = (sharingType: string) => {
    if (!user) {
      setSelectedRoomType(sharingType);
      setShowAuthModal(true);
      return;
    }
    
    // User is logged in, proceed with booking
    router.push(`/payment?propertyId=${property?.id}&sharingType=${encodeURIComponent(sharingType)}`);
  };

  // Handle authentication modal actions
  const handleLogin = () => {
    setShowAuthModal(false);
    router.push(`/login?redirect=/property/${propertyId}&sharingType=${encodeURIComponent(selectedRoomType)}`);
  };

  const handleSignup = () => {
    setShowAuthModal(false);
    router.push(`/signup?redirect=/property/${propertyId}&sharingType=${encodeURIComponent(selectedRoomType)}`);
  };
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6">
              ...
              <div className="md:ml-8 mt-6 md:mt-0">
                <button
                  ...
                  className="w-full px-8 py-4 bg-primary hover:bg-primary-dark text-white font-bold text-lg rounded-xl transition-all shadow-lg hover:shadow-xl flex items-center justify-center space-x-2"
                >
                  {!user && <FaLock className="text-sm" />}
                  <span>Choose Your Type</span>
                </button>
                {!user && (
                  <div className="mt-2 text-center">
                    <p className="text-xs text-amber-600 font-medium">Login required to complete booking</p>
                  </div>
                )}
            ...
            {(property as any).rooms && (property as any).rooms.length > 0 && (
              <div id="room-selection" className="mb-8">
                <div className="text-center mb-6">
                  ...
                  <p className="text-gray-600">Select a room type below to book</p>
                  {!user && (
                    <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                      <div className="flex items-center justify-center space-x-2 text-amber-700">
                        <FaLock className="text-sm" />
                        <span className="text-sm font-medium">Login required to complete booking</span>
                      </div>
                    </div>
                  )}
                </div>
                      ...
                      return (
                        ...
                        >
                          <div className="text-center">
                            ...
                            {availableRooms.length > 0 ? (
                              <button
                                onClick={() => handleBookingAttempt(sharingType)}
                                className="block w-full px-4 py-2 bg-primary hover:bg-primary-dark text-white font-semibold rounded-lg transition-all flex items-center justify-center space-x-2"
                              >
                                {!user && <FaLock className="text-sm" />}
                                <span>Book Now</span>
                              </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                Please login or create an account to book <span className="font-semibold text-primary">{selectedRoomType}</span> at this property.
              </p>
            </div>
            ...
            <div className="space-y-3">
              <button
                onClick={handleLogin}
                className="w-full px-6 py-3 bg-primary hover:bg-primary-dark text-white font-semibold rounded-xl transition-all flex items-center justify-center space-x-2"
              >
              ...
              <button
                ...
                className="w-full px-6 py-3 bg-white hover:bg-gray-50 text-primary border-2 border-primary font-semibold rounded-xl transition-all flex items-center justify-center space-x-2"
              >
                <FaUser className="text-sm" />
